...
export class DockerService {
  ...
  async listContainers(): Promise<Container[]> {
    try {
      ...
      return dockerContainers.map((dc) => this.normalizeContainer(dc));
    } catch (error) {
  ...
  async getPorts(id: string): Promise<Port[]> {
    try {
            ...
            if (Array.isArray(mappings) && mappings.length > 0) {
              mappings.forEach((mapping) => {
                ports.push({
                  ...
                  hostPort: mapping.HostPort ? parseInt(mapping.HostPort) : undefined,
      ...
      throw new DockerDaemonError(`Failed to get ports for container: ${id}`);
    }
  }
  ...
  private normalizeContainer(
    ...
    const status = this.normalizeStatus(dockerContainer.State?.Status || 'unknown');
    const name = dockerContainer.Name || dockerContainer.Names?.[0] || 'unknown';
    ...
    return {
      ...
      name: name.replace(/^\//, ''),
      status,
      image: dockerContainer.Image || dockerContainer.Config?.Image || 'unknown',
  ...
  private calculateMetrics(stats: any): ContainerMetrics {
    ...
    const cpuPercentage = systemDelta > 0
