...
export class WebSocketManager {
     ...
     */
    broadcastContainerStatusChanged(containerId: string, status: string): void {
        ...
        this.metricsInterval = setInterval(async () => {
            try {
                ...
                containers.forEach((container) => {
                    const previousStatus = this.lastContainerStates.get(
                        container.id,
                    );
                ...
                for (const container of containers) {
                    if (container.status === "running") {
                        try {
                            const metrics =
                                await this.dockerService.getContainerStats(
                                    container.fullId,
                                );
