...
export class WebSocketConnection {
  ...
  constructor(endpoint = '/api/metrics/stream', reconnectConfig?: Partial<ReconnectConfig>) {
  ...
  private attemptReconnect(): void {
    if (this.isIntentionallyClosed || this.reconnectAttempts >= this.reconnectConfig.maxAttempts) {
  ...
  private callMessageHandlers(message: WebSocketMessage): void {
    this.messageHandlers.forEach((handler) => {
      try {
        ...
        const err = error instanceof Error ? error : new Error('Handler error');
        this.callErrorHandlers(err);
      }
    });
  }
